...
* `allow_origins` - A list of origins that should be permitted to make cross-origin requests. eg. `['https://example.org', 'https://www.example.org']`. You can use `['*']` to allow any origin. Also allows `*` within the host to allow for wildcard subdomains. eg. `['https://*.example.org']`
